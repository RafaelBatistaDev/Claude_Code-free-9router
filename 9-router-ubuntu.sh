...
# Versão Ubuntu/Debian - Português do Brasil
# ==============================================================================
...
function install_bun() {
    ...
    if ! command -v curl &> /dev/null; then
        ...
        if ! sudo apt update -qq && sudo apt install -y -qq curl; then
            error "Falha ao instalar curl. Instale manualmente: sudo apt install curl"
            return 1
        fi
    fi
...
function validate_environment() {
    ...
    if [[ -f "/etc/os-release" ]]; then
        local os_info
        os_info=$(grep "^ID=" "/etc/os-release" | cut -d'=' -f2 | tr -d '"' | tr -d "'")
        debug "Sistema Operacional detectado: ${os_info}"
        if [[ "${os_info}" != "ubuntu" && "${os_info}" != "debian" && "${os_info}" != "linuxmint" && "${os_info}" != "pop" && "${os_info}" != "elementary" ]]; then
            warn "Este script foi otimizado para Ubuntu/Debian, mas pode funcionar em outras distribuições."
        fi
    fi
...
function install_global() {
    ...
    if cp "${SCRIPT_DIR}/9-router-ubuntu.sh" "${GLOBAL_BIN_DIR}/9router" && \
...
function show_help() {
    cat << 'HELP_EOF'
╔══════════════════════════════════════════════════════════════╗
║                    9Router CLI - Ubuntu/Debian               ║
╚══════════════════════════════════════════════════════════════╝

Uso: ./9-router-ubuntu.sh [opções]
...
SISTEMAS SUPORTADOS:
  ...
  • Pop!_OS
  • Elementary OS
  • Outros baseados em Debian/Ubuntu
HELP_EOF
}
...
function delegate_to_9router() {
    ...
    if [[ -f "${bun_bin_path}/9router" ]]; then
        ...
        error "Execute a instalação primeiro: ./9-router-ubuntu.sh --install"
        exit 1
    fi
}
...
function main() {
    ...
    case "${1}" in
        ...
        *)
            ...
            echo "💡 Execute primeiro: ./9-router-ubuntu.sh --install"
            echo ""
            delegate_to_9router "${@}"
            ;;
    esac
